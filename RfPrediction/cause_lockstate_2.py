...
for direction in ['Z','X','Y']:
    ...
    H1_binary_file = open('/home/eric.coughlin/gitrepo/seismon/RfPrediction/data/LHO_O1_predicted_binary_{0}{1}.txt'.format(direction, rms_toggle), 'w')

    for eq_time, pw_arrival,predicted_peak_ground_velocity,eq_mag,eq_distance,eq_depth, lockloss in zip(eq_time_list, pw_arrival_list,predicted_peak_ground_velocity_list,eq_mag_list,eq_distance_list,eq_depth_list, locklosslist):
        if lockloss == "Y":
            lockloss_binary = '1'
            H1_binary_file.write('{0:^20} {1:^20} {2:^20} {3:^20} {4:^20} {5:^20} {6:^20} \n'.format(eq_time,pw_arrival,predicted_peak_ground_velocity,eq_mag,eq_distance,eq_depth,lockloss_binary))
        elif lockloss == "N":
            lockloss_binary = '0'
            H1_binary_file.write('{0:^20} {1:^20} {2:^20} {3:^20} {4:^20} {5:^20} {6:^20}\n'.format(eq_time,pw_arrival,predicted_peak_ground_velocity,eq_mag,eq_distance,eq_depth,lockloss_binary))
        else:
...
for direction in ['Z','X','Y']:
    ...
    L1_binary_file = open('/home/eric.coughlin/gitrepo/seismon/RfPrediction/data/LLO_O1_predicted_binary_{0}{1}.txt'.format(rms_toggle, direction), 'w')
    for eq_time, pw_arrival,predicted_peak_ground_velocity,eq_mag,eq_distance,eq_depth, lockloss in zip(eq_time_list, pw_arrival_list,predicted_peak_ground_velocity_list,eq_mag_list,eq_distance_list,eq_depth_list, locklosslist):
        if lockloss == "Y":
            lockloss_binary = '1'
