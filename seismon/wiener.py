...
def wiener(params, target_channel, segment):
    ...
    bins,originalSpecvar = seismon.utils.spectral_histogram(originalSpecgram)
    original_spectral_variation_50per = seismon.utils.spectral_percentiles(originalSpecvar,bins,50)
    bins,residualSpecvar = seismon.utils.spectral_histogram(residualSpecgram)
    residual_spectral_variation_50per = seismon.utils.spectral_percentiles(residualSpecvar,bins,50)
    bins,FFSpecvar = seismon.utils.spectral_histogram(FFSpecgram)
    FF_spectral_variation_50per = seismon.utils.spectral_percentiles(FFSpecvar,bins,50)
